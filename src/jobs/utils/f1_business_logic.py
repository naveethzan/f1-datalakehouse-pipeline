...
class F1BusinessLogic:
    ...
    def calculate_championship_gap(driver_points: float, leader_points: float) -> float:
        ...
        """
        return max(0.0, leader_points - driver_points)
    ...
    @staticmethod
    def calculate_lap_consistency_score(lap_times: List[float]) -> float:
        """
        Calculate lap time consistency score (coefficient of variation).
        
        Args:
            lap_times: List of lap times in seconds
            
        Returns:
            Consistency score (stddev/mean) - lower is more consistent
        """
        if not lap_times or len(lap_times) < 2:
            return 0.0
        
        # Remove outliers (pit laps, safety car laps)
        # Simple outlier removal: remove times > 1.5x median
        ...
        return std_dev / mean_time if mean_time > 0 else 0.0
    ...
    @staticmethod
    def add_recent_form_points(df: DataFrame,
                             points_col: str = "race_points",
                             partition_col: str = "driver_number",
                             order_col: str = "race_date",
                             num_races: int = 3) -> DataFrame:
        """
        Add recent form points (last N races).
        """
        window_spec = F1BusinessLogic.get_recent_form_window_spec(
            [partition_col], [order_col], num_races
        )
        
        return df.withColumn(f"last_{num_races}_races_points", 
                           sum(points_col).over(window_spec))
    ...
    @staticmethod
    def add_q3_appearances_total(df: DataFrame,
                               q3_col: str = "made_q3",
                               partition_col: str = "driver_number",
                               order_col: str = "race_date") -> DataFrame:
        """
        Add running total of Q3 appearances (qualifying sessions only).
        """
        window_spec = F1BusinessLogic.get_running_total_window_spec([partition_col], [order_col])
        
        # Convert boolean to integer for sum
        q3_int = when(col(q3_col) == True, 1).otherwise(0)
        
        return df.withColumn("q3_appearances_season_total", 
                           sum(q3_int).over(window_spec))
    
    @staticmethod
    def add_teammate_comparison(df: DataFrame,
                              comparison_col: str,
                              team_col: str = "team_name",
                              race_col: str = "race_date",
                              session_type: str = "Race") -> DataFrame:
        """
        Add session-specific teammate comparison flag.
        """
        window_spec = F1BusinessLogic.get_teammate_comparison_window_spec(team_col, race_col)
        
        # Get teammate's value (assuming 2 drivers per team)
        teammate_value = when(
            row_number().over(window_spec) == 1,
            lead(comparison_col, 1).over(window_spec)
        ).otherwise(
            lag(comparison_col, 1).over(window_spec)
        )
        
        # For positions, lower is better
        comparison_col_name = f"beat_teammate_{comparison_col.replace('_position', '')}"
        
        return df.withColumn(comparison_col_name,
                           when(col(comparison_col) < teammate_value, True)
                           .when(col(comparison_col) > teammate_value, False)
                           .otherwise(None))
    ...
    def add_season_progression(df: DataFrame,
                             ...
                             session_type: str = "Race") -> DataFrame:
        """
        Add season progression columns (running totals, recent form) - race sessions only.
        """
        if session_type != F1BusinessLogic.SESSION_TYPE_RACE:
            logger.warning("Season progression only calculated for race sessions")
            return df
        
        df_with_total = F1BusinessLogic.add_running_points_total(
            df, points_col, driver_col, date_col, session_type
        )
        
        df_with_form = F1BusinessLogic.add_recent_form_points(
            df_with_total, points_col, driver_col, date_col
        )
    ...
    def add_qualifying_season_metrics(df: DataFrame,
        ...
        df_with_q3 = F1BusinessLogic.add_q3_appearances_total(
            df_with_avg, "made_q3", driver_col, date_col
        )
